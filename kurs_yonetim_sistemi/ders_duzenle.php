...
if ($id <= 0) {
    setMessage('error', 'Geçersiz ders ID.');
    header("Location: dersler.php");
    exit;
}

// Ders bilgilerini al
$dersQuery = "SELECT * FROM dersler WHERE ders_id = $id";
$dersResult = $conn->query($dersQuery);

if ($dersResult->num_rows === 0) {
    setMessage('error', 'Ders bulunamadı.');
    header("Location: dersler.php");
    exit;
}

$ders = $dersResult->fetch_assoc();
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($ders_adi)) {
        ...
        $stmt = $conn->prepare("UPDATE dersler SET ders_adi = ?, aciklama = ? WHERE ders_id = ?");
        $stmt->bind_param("ssi", $ders_adi, $aciklama, $id);

        if ($stmt->execute()) {
            $success = "Ders bilgileri başarıyla güncellendi.";
            // Ders bilgilerini yenile
            $dersResult = $conn->query($dersQuery);
            $ders = $dersResult->fetch_assoc();
        } else {
            $error = "Ders güncellenirken bir hata oluştu: " . $stmt->error;
        }
...
<head>
    ...
    <title>Ders Düzenle - Kurs Yönetim Sistemi</title>
                    ...
                    <div class="card-body">
                        <form method="POST" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"] . "?id=" . $id); ?>" class="needs-validation" novalidate>
                            <div class="mb-3">
                                ...
                                <input type="text" class="form-control" id="ders_adi" name="ders_adi" value="<?php echo $ders['ders_adi']; ?>" required>
                                <div class="invalid-feedback">Ders adı zorunludur.</div>
                            </div>

                            <div class="mb-3">
                                <label for="aciklama" class="form-label">Açıklama</label>
                                <textarea class="form-control" id="aciklama" name="aciklama" rows="4"><?php echo $ders['aciklama']; ?></textarea>
                            </div>
